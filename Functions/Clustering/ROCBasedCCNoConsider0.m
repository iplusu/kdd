function [ CCTPR, CCFPR, TH ] = ROCBasedCCNoConsider0( day )
...
%   Detailed explanation goes here
    ...
    for cc_th = 0.0:0.01:1.0
        TPi = inDshield <= cc_th & inDshield > 0.0;
        TPi = sum(TPi, 2) >= 1;
        TPi = sum(TPi);
        
        FNi = inDshield <= cc_th & inDshield > 0.0;
        FNi = sum(FNi, 2) < 1;
        FNi = sum(FNi);
        
        FPi = nonDshield <= cc_th & nonDshield > 0.0;
        FPi = sum(FPi, 2) >= 1;
        FPi = sum(FPi);

        TNi = nonDshield <= cc_th & nonDshield > 0.0;
        TNi = sum(TNi, 2) < 1;
        TNi = sum(TNi);
