function [ AvgTPR, AvgTPRList, AvgFPR, AvgFPRList, TH ] = getAverageDegreeBase2( threshold )
    AvgTPRList = [];
    AvgFPRList = [];
    ...
    tmpList = nonDshieldDegree_summation / 14;
    tmpList = find(tmpList < threshold);
    for idx = 1:length(tmpList)
    ...
    %     FPi = sum(nonDshieldTrust1(:,14) <= idx);
        ...
        A_TPi = sum(dshieldTrust1(:,14) <= idx);
        A_FPi = sum(nonDshieldTrust1(:,14) <= idx);
        A_FNi = sum(dshieldTrust1(:,14) > idx);
        A_TNi = sum(nonDshieldTrust1(:,14) > idx);
        Pi = A_TPi + A_FNi;
        Ni = A_TNi + A_FPi;
    %      Pi = TPi + FPi;
    %      Ni = TNi + FNi;
