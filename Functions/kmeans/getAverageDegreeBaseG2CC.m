function [ AVCCTTPR, AVCCTFPR, AVCCTH ] = getAverageDegreeBaseG2CC( )
...
%   Detailed explanation goes here
    ...
    for idx = 0.01:0.01:1.0
        TPi = sum(DShieldCCGrp2, 2) / 14;
        TPi = sum(TPi <= idx & TPi > 0);
        
        FNi = sum(DShieldCCGrp2, 2) / 14;
        FNi = sum(FNi > idx | FNi == 0);
        
        FPi = sum(nonDShieldCCGrp2, 2) / 14;
        FPi = sum(FPi <= idx & FPi > 0);
        
        TNi = sum(nonDShieldCCGrp2, 2) / 14;
