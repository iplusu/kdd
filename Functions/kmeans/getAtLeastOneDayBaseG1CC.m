function [ ALOCTTPR, ALOCTFPR, ALOCTH ] = getAtLeastOneDayBaseG1CC( )
...
%   Detailed explanation goes here
    ...
    for idx = 0.01:0.01:1.0
        TPi = DShieldCCGrp1 <= idx & DShieldCCGrp1 > 0.0;
        TPi = sum(TPi, 2);
        TPi = sum(TPi >= 1);
        
        FNi = DShieldCCGrp1 <= idx & DShieldCCGrp1 > 0.0;
        FNi = sum(FNi, 2);
        FNi = sum(FNi < 1);
        
        FPi = nonDShieldCCGrp1 <= idx & nonDShieldCCGrp1 > 0;
        FPi = sum(FPi, 2);
        FPi = sum(FPi >= 1);
        
        TNi = nonDShieldCCGrp1 <= idx & nonDShieldCCGrp1 > 0;
        TNi = sum(TNi, 2);
        TNi = sum(TNi < 1);
